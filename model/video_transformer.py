...
def attn(q, k, v):
    ...
    #a matrix mutliplication for the channel dimension
    attn = sim.softmax(dim=-1)
    out = einsum('b i j, b j d -> b i d', attn, v)
    #a matrix mutliplication for width dimension
    #(q*k)*v
    return out
...
class VideoPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768,
        ...
        #M*N M：num_frames N:patch numbers in every image
        ...
        freeze(self)
        #B*M,H,W,C->num_patch,patch,patch,embed_dim
    def forward(self, x):
        B, F, C, H, W = x.shape
...
class VarAttention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, **einops_dims):
        ...
        #slice 3 parts in the last dimenstion
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (q, k, v))
        #a flexible way to change the dimension
        q *= self.scale
...
class SpaceTimeBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()#a always same function
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        #partial can make the layernorm with the same fix parameter
        #layer normalization
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, self.patches_per_frame + 1,
                        embed_dim))  # remember to take pos_embed[1:] for tiling over time,+1 for class token
        ...
        #initialize the parameter with a uniform distribution, and cut it with a std 0.02
    ...
    def forward_features(self, x):
        ...
        #second parameter in flatten is start dim
        #B*num_frames,num_patch for every frame,embedded dim; every patch size project into an embedded dim
        x = x.reshape(b, -1, self.patch_embed.embed_dim)
        #-1 is M*N M is number of frame N is the patch numbers
        BF = x.shape[0]
        cls_tokens = self.cls_token.expand(BF, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks expand as thr final dimension
        x = torch.cat((cls_tokens, x), dim=1)
        # positional embed needs to be tiled for each frame (this does [1,2,3] --> [1,2,3,1,2,3]...)
        cls_embed = self.pos_embed[:, 0, :].unsqueeze(1)
        #notice: class token doesn't need the position and temporal embedded at the same time
