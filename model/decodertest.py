...
class FirstTransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, batch_size=4,token_num=2,dim_feedforward=2048, dropout=0.1,
        ...
        self.activation = _get_activation_fn(activation)
        self.normalize_before = normalize_before
        self.tgt=nn.Parameter(torch.zeros(token_num,batch_size,768))
    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else tensor + pos
...
if __name__ == "__main__":
    k=FirstTransformerDecoderLayer(768, 12)
    d=TransformerDecoderLayer(d_model=768,nhead=12)
    m=TransformerDecoder(first_decoder_layer=k,decoder_layer=d,num_layers=4)
     
    # tgt=torch.rand((2,1,768))
    memory=torch.rand((1,4,768))
    print(m(memory).shape)
    print(m(memory)[0].shape)
