        ...
        for batch_idx, data_li in enumerate(zip(*self.data_loader)):
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                with torch.set_grad_enabled(True):
                    vid_scores1,vid_scores2 = self.model(data, video_only=True)
                    # vid_scores = self.allgather(vid_scores, self.n_gpu, self.args)
                    # cls_label = self.allgather(cls_label, self.n_gpu, self.args)
                    loss1 = self.loss(vid_scores1, cls_label)
                    loss2 = self.loss(vid_scores2, cls_label)
                    loss = loss1+loss2
                loss.backward()
                ...
                if batch_idx % self.log_step == 0 and self.args.rank == 0:
                    self.logger.info('Train Epoch: {} dl{} {} Loss1: {:.6f}'.format(
                        epoch,
                        dl_idx,
                        self._progress(batch_idx, dl_idx),
                        loss1.detach().item()))
                    self.logger.info('Train Epoch: {} dl{} {} Loss2: {:.6f}'.format(
                        epoch,
                        dl_idx,
                        self._progress(batch_idx, dl_idx),
                        loss2.detach().item()))
