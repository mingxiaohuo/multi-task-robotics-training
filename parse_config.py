...
class ConfigParser:
    def __init__(self, args, options='', timestamp=True, test=False, eval_mode=None):
        ...
        if args.resume is None:
            ...
            if eval_mode == "epic":
                resume_cfg_fname = Path('/mnt/hdd1/ego4d_proj/mingxiaohuo_ego4d/EgoVLP/configs/eval/epic.json')
            if eval_mode == "charades":
                resume_cfg_fname = Path('/mnt/hdd1/ego4d_proj/mingxiaohuo_ego4d/EgoVLP/configs/eval/charades.json')
            if eval_mode == "nlq":
                resume_cfg_fname = Path('/mnt/hdd1/ego4d_proj/mingxiaohuo_ego4d/EgoVLP/configs/eval/nlq.json')
            if eval_mode == "mq":
                resume_cfg_fname = Path('/mnt/hdd1/ego4d_proj/mingxiaohuo_ego4d/EgoVLP/configs/eval/mq.json')
    ...
    def initialize(self, name, module,  *args, index=None, **kwargs):
        #*args:just values group; **kwargs:key and value dictionary; they both save the other variables
        """
        ...
        signature = inspect.signature(getattr(module, module_name).__init__)        #._init_ can represent the class's initialize function def_init_()
        print(module_name)
...
def _update_config(config, options, args):
    for opt in options:
        ...
        #getattr compared with the traditional ., it can deal with more complex situations
