...
class TextVideoDataset(Dataset):
    def __init__(self,
        ...
        self.text_params = text_params #input:text
        self.video_params = video_params
        # check for environment variables
        self.data_dir = os.path.expandvars(data_dir)
        #if not exist, it will keep the same
    ...
    def _get_video_lens(self):
        ...
        for idx, row in self.metadata.iterrows():
            #iterrows: return all the rows idx and contents
    ...
    def _fix_temporal_samples(self):
        ...
        self.metadata['frame_intervals'] = self.metadata['vlen'].apply(
            ...
            #define a new col, and every x is created by np.linspace
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            if video_loading == 'strict':
                ...
                #get a new image with size res*res, rgb all 0
...
def sample_frames(num_frames, vlen, sample='rand', fix_start=None):
    ...
    #have num_frame video len, getting samples interval
...
def sample_frames_clips(start, end, vlen, acc_samples):
    ...
    for idx, interv in enumerate(intervals[:-1]):
        ...
        #// a divide to int, return a single frame id 
    return frame_idxs
...
def read_frames_cv2(video_path, num_frames, sample='rand', fix_start=None):
    ...
    #if 0, get the camera of the laptop; else get the video in the path
    assert (cap.isOpened())
    vlen = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    #cap.get(cv2.CAP_FRAME_COUNT) a kind of get numbers of frames
...
def read_frames_cv2_egoclip(video_path_1, video_path_2, num_frames, sample,
    ...
    for index in frame_idxs:
        _index = index % (600 * 30)
        #each video is 18000 sec, but do not know why egoclip 2 videos
...
def read_frames_cv2_epic(video_path, start_frame, stop_frame, num_frames, sample='rand', fix_start=None):
    ...
    for index in frame_idxs:
        ...
        #zfill makes all index be 10 digits
