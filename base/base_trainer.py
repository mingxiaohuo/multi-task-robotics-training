...
class BaseTrainer:
    ...
    def __init__(self, model, loss, metrics, optimizer, config, writer=None, init_val=False):
        ...
        if len(device_ids) > 1:
            ...
            #create model on different gpus
        ...
        if self.monitor == 'off':
            ...
            #mnt_mode max/min mnt_metic ie. val_loss0
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        #save the information in the logger file
            ...
            for key, value in result.items():
              if self.args.rank == 0:
                if key == 'metrics':
                    log.update({mtr.__name__: value[i]
                                for i, mtr in enumerate(self.metrics)})
                                #what is the matrics, and how that log into the data
                elif key == 'val_metrics':
                    log.update({'val_' + mtr.__name__: value[i]
                                for i, mtr in enumerate(self.metrics)})
                elif key == 'nested_val_metrics':
